...
const { validateDateOfBirth, calculateAge } = require('../utils/dateUtils');
...
router.post('/start', requirePermission('employees', 'create'), async (req, res) => {
  try {
    ...
    if (employee_data.date_of_birth && !validateDateOfBirth(employee_data.date_of_birth)) {
      return res.status(400).json({ 
        error: 'Invalid date of birth. Must be a valid date in the past.' 
      });
    }
    ...
    const employeeResult = await runQuery(`
      INSERT INTO employees (
        ...
        date_of_birth, gender, nationality, job_description, employment_type, education_qualifications, relevant_experience
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      employeeId,
      ...
      1, // is_active
      employee_data.date_of_birth || null,
...
router.get('/processes', requirePermission('employees', 'read'), async (req, res) => {
  try {
    ...
    // Calculate age for each process from date_of_birth
    const processesWithAge = processes.map(process => ({
      ...process,
      age: calculateAge(process.date_of_birth)
    }));

    res.json({ success: true, processes: processesWithAge });
  } catch (error) {
