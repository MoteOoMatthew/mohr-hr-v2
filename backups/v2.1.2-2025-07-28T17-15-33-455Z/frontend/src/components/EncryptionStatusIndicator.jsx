...
const EncryptionStatusIndicator = () => {
  const { e2eeStatus, needsE2EEReinit } = useAuth();
  ...
  const getStatusColor = () => {
    if (!encryptionStatus.initialized) {
      return 'text-red-600 bg-red-50 border-red-300 hover:bg-red-100 transition-colors animate-pulse';
    }
  ...
  const handleStatusClick = () => {
    if (needsE2EEReinit()) {
      // Trigger the E2EE modal by dispatching a custom event
